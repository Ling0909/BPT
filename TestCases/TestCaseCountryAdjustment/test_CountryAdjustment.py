...
class TestCountryAdjustment:
    @pytest.mark.parametrize('data', dpp.datas_CountryAdjustment)
    def testCase(self,for_policyList_class,data):
        #获取portfolio页面中的baseI和fundingOthers1_Value
        CostFunding_Value=for_policyList_class[1].find_by_portfolioNo(dpl.datas_CountryAdjustment['CostFundingName'],data['portfolioNo'],data['lineCode'])
        #获取portfolio页面需要的月份
