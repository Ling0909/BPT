...
@pytest.mark.OTH
class TestCEOFundingOthers3:
    @pytest.mark.parametrize('data',dpp.datas_GEOFundingOthers3)
    def test_CEOFundingOthers3(self,for_policyList_class,data):

        #获取portfolio页面中的baseI和fundingOthers1_Value
        CostFunding_Value=for_policyList_class[1].find_by_portfolioNo(dpl.datas_GEOFundingOthers3['CostFundingName'],data['portfolioNo'],data['lineCode'])
        #获取portfolio页面需要的月份
