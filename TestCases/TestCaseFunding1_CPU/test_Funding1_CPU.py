...
@pytest.mark.CPU
@pytest.mark.smoke3
class TestFunding1_CPU:
    @pytest.mark.parametrize('data', dpp.datas_Funding1_CPU)
    def test_funding1_CPU(self,for_policyList_class,data):

        #获取portfolio页面中的baseI和fundingOthers1_Value
        CostFunding_Value=for_policyList_class[1].find_by_portfolioNo(dfm.datas_Funding1_CPU['CostFundingName'],data['portfolioNo'],data['lineCode'])
        #获取portfolio页面需要的月份
