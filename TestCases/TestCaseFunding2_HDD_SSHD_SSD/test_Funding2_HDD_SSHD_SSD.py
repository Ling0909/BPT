...
@pytest.mark.CPU
@pytest.mark.ssd
class TestFunding2_HDD_SSHD_SSD:
    @pytest.mark.parametrize('data', dpp.datas_Funding2_HDD_SSHD_SSD)
    def test_funding2_HDD_SSHD_SSD(self,for_policyList_class,data):

        #获取portfolio页面中的baseI和fundingOthers1_Value
        CostFunding_Value=for_policyList_class[1].find_by_portfolioNo(dfm.datas_Funding2_HDD_SSHD_SSD['CostFundingName'],data['portfolioNo'],data['lineCode'])
        #获取portfolio页面需要的月份
